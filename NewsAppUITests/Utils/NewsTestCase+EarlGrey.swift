...
extension NewsTestCase {
    ...
    open func open(viewController: UIViewController,
        ...
        if modally {
            UIWindow.keyNewsWindow.set(rootViewController: UIViewController())
            UIWindow.keyNewsWindow.rootViewController?.present(viewControllerToOpen,
                                                               animated: true,
                                                               completion: nil)
        } else {
            UIWindow.keyNewsWindow.set(rootViewController: viewControllerToOpen)
        }
    }
    ...
    @discardableResult func waitToBeVisible(_ viewControllerType: UIViewController.Type) -> Bool {
        let success = GREYCondition.init(name: "Wait ViewController to be visible", block: { () -> Bool in
            ...
            guard let rootViewController = UIWindow.keyNewsWindow.rootViewController else { return false }
